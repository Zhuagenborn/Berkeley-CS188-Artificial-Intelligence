...
def evaluate(generateSolutions, testRoot, moduleDict, exceptionMap=ERROR_HINT_MAP,
    ...
    for q in test_subdirs:
        ...
        tests = list(filter(lambda t: re.match(
            '[^#~.].*\.test\Z', t), os.listdir(subdir_path)))
        tests = list(map(lambda t: re.match(
            '(.*)\.test\Z', t).group(1), tests))
...
def generatePublicTests(moduleDict, privateRoot='private_test_cases', publicRoot='test_cases'):
    ...
    for q in sorted(os.listdir(privateRoot)):
        ...
        tests = list(filter(lambda t: re.match(
            '[^#~.].*\.test\Z', t), os.listdir(private_subdir_path)))
        tests = list(map(lambda t: re.match(
            '(.*)\.test\Z', t).group(1), tests))
        for t in sorted(tests):
            ...
            test_out_file = os.path.join(
                public_subdir_path, '%s.test_output' % t)
