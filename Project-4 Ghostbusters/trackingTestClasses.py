        ...
        if l2 > self.L2Tolerance:
            if self.errors == 0:
                ...
                header = '%10s%5s%-25s%-25s\n' % ('key:',
                                                  '', 'student', 'reference')
                detail = '\n'.join(list(map(lambda x: '%9s:%5s%-25s%-25s' % (
                    x, '', dist[x], refDist[x]), set(list(dist.keys()) + list(refDist.keys())))))
                print(dist.items())
                print(refDist.items())
...
class SeededRandomGhostAgent(Agent):
    ...
    def sample(self, distribution, values=None):
        if type(distribution) == util.Counter:
            items = [(k, distribution[k])
                     for k in fixed_order if k in distribution]
